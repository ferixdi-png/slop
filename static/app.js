...
async function loadRadar(){
  const host=$('#radarRows'); if(!host)return;
  host.innerHTML='<div class="empty">Загружаю радар…</div>';
  try{
    const r=await fetch('/api/radar'); const rows=await r.json();
    if(!r.ok) throw new Error('Не удалось загрузить радар');
    if(!rows.length){host.innerHTML='<div class="empty">Пока нет отобранных роликов. Нужен первый запуск синхронизации радара.</div>';return;}
    host.innerHTML=rows.map((x,i)=>{
      ...
      return `<div class="radar-card">
        ...
        <div class="radar-main"><div class="radar-title">${hot} ${escapeHtml(x.hook||x.scene_description||'Подходящая AI-сценка')}</div><div class="radar-meta">@${escapeHtml(x.creator)} · ${Number(x.duration_sec||0).toFixed(1)} сек · ${Number(x.hours_since_publish||0).toFixed(1)} ч назад</div><div class="radar-desc">${escapeHtml(x.scene_description||'')}</div></div>
  ...
  }catch(e){host.innerHTML=`<div class="error">${escapeHtml(e.message)}</div>`;}
}
async function analyzeRadar(id){
  showAnalyzer();
  await runAnalysis(()=>fetch(`/api/radar/${id}/analyze`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({owned_or_licensed:$('#rights')?.checked||false})}));
}
$('#refreshRadar')?.addEventListener('click',()=>loadRadar());

function formatNum(v){
  const n=Number(v||0); if(n>=1e6)return (n/1e6).toFixed(n>=1e7?0:1)+' млн'; if(n>=1e3)return (n/1e3).toFixed(n>=1e5?0:1)+' тыс'; return Math.round(n).toLocaleString('ru-RU');
}
function escapeHtml(v=''){return String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));}
function escapeAttr(v=''){return escapeHtml(v).replace(/\n/g,'&#10;');}

async function loadStatus(){
  const el=$('#serviceStatus'); if(!el)return;
  try{
  ...
  }catch(_){ el.textContent='статус недоступен'; }
}

async function syncRadar(){
  const key=($('#radarKey')?.value||'').trim();
  const out=$('#radarSyncResult');
  if(!key){ if(out) out.textContent='Введите внутренний RADAR_SYNC_KEY из переменных Render.'; return; }
  const btn=$('#syncRadar'); if(btn){btn.disabled=true; btn.textContent='ИЩУ РОЛИКИ…';}
  if(out) out.textContent='Запущен поиск по запросам хештегам и отслеживаемым авторам. Это может занять несколько минут.';
  try{
    const r=await fetch('/api/radar/sync',{method:'POST',headers:{'X-Radar-Key':key}});
  ...
  finally{ if(btn){btn.disabled=false; btn.textContent='ЗАПУСТИТЬ ПОИСК';} }
}
